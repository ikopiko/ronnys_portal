...
export default {
  ...
  data() {
    return {
      ...
      branchURL: null,
      json_fields: {
        "User Name": "total_name",
        "Branch Name": "branch_name",
        "Total Work Time": "total_worktime",
        "Total Break Time": "total_breaktime",
        "Total Break Over Time": "total_breackover",
     },
      supplyHeaders: [
        {
          value: "total_name",
        ...
        {
          value: "total_worktime",
          text: "Total Work Time",
          sortable: true,
        },
        {
          value: "total_breaktime",
          text: "Total Break Time",
          sortable: true,
        },
        {
          value: "total_breackover",
          text: "Total Break Over Time",
          sortable: true,
        },
      ...
      var role = vm.$store.state.authfack.user.role;
       if (role == "admin" || role == "financialManager" || role == "operationalManager"
        || role == "branchManager" || role == "viceManager" || role == "hrManager") {
         vm.$router.push({path: "/reports/timetablesum"}).catch(()=>{});
       }
       else {
  ...
  methods: {
    ...
    formatRange(val){
        ...
        this.startDateString = val[0];
        this.endDateString = val[1];
    },
     sumField(key) {
        const sum =  this.reportList.reduce((a, b) => a*1 + (b[key]*1 || 0), 0)
    ...
    getUserList(){
        axios
          .request({
            method: "post",
            url: this.$hostname + "tabel/userlist",
            headers: {
              Authorization: "Bearer " + this.TOKEN,
            },
            // data: {
    ...
    getReport() {
      ...
      if (this.$refs.searchForm.validate()) {
        ...
        axios
          .request({
            method: "post",
            url: this.$hostname + "tabel/historyrange",
            headers: {
              Authorization: "Bearer " + this.TOKEN,
            },
            data: {
          ...
          .then((response) => {
            this.loader = false;
            // eslint-disable-next-line no-console
            this.reportList = this.json_data = response.data.data;

            this.reportList.forEach(x => {
                x.total_worktime = Number(x.total_worktime).toFixed(2);
                x.total_breaktime = Number(x.total_breaktime).toFixed(2);
                x.total_breackover = Number(x.total_breackover).toFixed(2);
                // x.totalWorktime = parseFloat(this.tota);
            });
            this.json_data = this.reportList;
